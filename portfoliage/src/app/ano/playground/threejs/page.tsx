...
const ReactLenis: React.FC<{ children?: React.ReactNode; root?: boolean }> = ({ children }) => <>{children}</>;
import ZoomOutSection from "@/components/ano/playground/ZoomOutSection";


export default function ThreejsAnoPlayground() {
  return (
    <ReactLenis root>
    <div className="w-full relative overflow-hidden">
      <div className="h-screen">
        <Canvas camera={{ position: [0, 0, 50], fov: 25 }} className="h-screen">
            <Lens>
                <Images />
            </Lens>
        </Canvas>
...
function Images() {
  ...
  return (
    <group ref={group}>
      <Svg position={[-15, 10, 0]} scale={0.01} src="/assets/ano/playground/my-drawing.svg" />
    </group>
  )
}
